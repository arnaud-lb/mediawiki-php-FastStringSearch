...
void *kws_emalloc_wrapper(size_t size);
void kws_efree_wrapper(void *ptr);

#define NCHAR (UCHAR_MAX + 1)
#define obstack_chunk_alloc kws_emalloc_wrapper
#define obstack_chunk_free kws_efree_wrapper
...
kwsalloc (char const *trans)
{
  ...
  kwset = (struct kwset *) kws_emalloc_wrapper(sizeof (struct kwset));
  if (!kwset)
    return NULL;
...
bmexec (kwset_t kws, char const *text, size_t size)
{
  ...
  if (len == 1)
    {
      tp = (char *)memchr (text, kwset->target[0], size);
      return tp ? tp - text : -1;
    }
